...
class Scene:
    ...
    def from_manual(
        ...
        """Construct a lightweight scene using manually provided data."""

        scene = cls.__new__(cls)
        scene.model_path = None
        scene.loaded_iter = load_iter
        scene.triangles = triangles
        scene.cameras_extent = cameras_extent

        scales = resolution_scales or [1.0]
        train_cameras = list(train_cameras)
        test_cameras = list(test_cameras) if test_cameras is not None else []

        scene.train_cameras = {scale: train_cameras for scale in scales}
        scene.test_cameras = {scale: test_cameras for scale in scales}
